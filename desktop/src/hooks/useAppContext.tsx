...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const { config, isConfigLoading, configError, refreshConfig } =
    useConfig(enabled);
  ...
  useEffect(() => {
    if (config?.currentWorkspace && workspaces && workspaces.length > 0) {
      ...
      const configWorkspaceExists = workspaces.some(
        (w) => w.name === config.currentWorkspace,
      );
