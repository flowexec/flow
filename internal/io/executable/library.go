...
func workspacePageRows(
	...
	for _, ws := range wsList {
		...
		rows = append(rows, views.TableRow{
			Data: padRow([]string{
				name, fmt.Sprintf("%d", count), tags, common.ShortenPath(ws.Location()),
			}, wsRowKindWS, ws.AssignedName(), ""),
	...
	return views.LibraryPage{
		...
		Factory: func(render *types.RenderState, _ []views.PageSelection) (tea.Model, []types.KeyCallback) {
			...
			columns := []views.TableColumn{
				{Title: fmt.Sprintf("Workspaces (%d)", len(wsList)), Percentage: 32},
				{Title: "#", Percentage: 8},
				{Title: "Tags", Percentage: 30},
				{Title: "Location", Percentage: 30},
			}
...
func executablePageKeys(
	...
	selectedExec := func() *executable.Executable {
		...
		// data[0] is the full ref string ("verb workspace/ns:name")
		ref := executable.Ref(data[0])
		ex, err := visible.FindByVerbAndID(ref.Verb(), ref.ID())
		if err != nil {
			return nil
		}
		return ex
	...
	return []types.KeyCallback{
		...
		{Key: "c", Label: "copy ref", Callback: func() error {
			...
			common.CopyToClipboard(container, ex.Ref().String(), "copied reference to clipboard")
			return nil
		}},
	}
}
...
func executablesPage(
	...
	return views.LibraryPage{
		...
		Factory: func(render *types.RenderState, selections []views.PageSelection) (tea.Model, []types.KeyCallback) {
			...
			slices.SortFunc(visible, sortByID)

			columns := []views.TableColumn{
				{Title: fmt.Sprintf("Executables (%d)", len(visible)), Percentage: 40},
			...
			for _, ex := range visible {
				...
				// Visible: [0]=ID, [1]=Flowfile, [2]=Tags; Hidden: [3]=Verb (used by key callbacks)
				rows = append(rows, views.TableRow{
					Data: []string{ex.Ref().String(), flowfile, tags, string(ex.Verb)},
				})
			}
			table := views.NewTable(render, columns, rows, views.TableDisplayFull)
...
func executableDetailPage(
	...
	return views.LibraryPage{
		...
		Factory: func(render *types.RenderState, selections []views.PageSelection) (tea.Model, []types.KeyCallback) {
			...
			if len(selections) > 1 && len(selections[1].Data) >= 4 {
				row := selections[1].Data
				pageFilter := executableFilterFromSelection(filter, selections)
				visible := applyFilter(execs, pageFilter)
				// row[0] is the full ref string ("verb workspace/ns:name");
				// parse it as a Ref to extract just the ID for lookup.
				ref := executable.Ref(row[0])
				if found, err := visible.FindByVerbAndID(ref.Verb(), ref.ID()); err == nil {
					ex = found
				}
			}
			if ex == nil {
			...
			view := NewExecutableView(ctx, ex, runFunc)
